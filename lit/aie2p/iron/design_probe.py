"""IRON design for MAC probe kernel.

Same structure as design.py but uses the mac_probe kernel.
Transfers one A tile (4x32 i16), one B tile (4x32 i16) to compute,
calls mac_probe (single MAC + SRS), transfers C tile (16 i32) back.

Usage (requires ironenv):
  python3 design_probe.py > build/design.mlir
"""
...
# --- Kernel declaration ---
probe_kernel = Kernel("mac_probe", "mac_probe.o", [a_ty, b_ty, c_ty])
...
worker = Worker(core_fn, [of_a.cons(), of_b.cons(), of_c.prod(), probe_kernel])
